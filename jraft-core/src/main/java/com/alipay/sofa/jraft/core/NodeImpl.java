...
import com.alipay.sofa.jraft.error.*;
...
public class NodeImpl implements Node, RaftServerService {
    ...
    /**
     * 与日志数据存储模块的设计相类似，JRaft 针对快照数据存储模块同样采用了操作与存储相分离的策略，
    ...
    /**
     * 核心信息是currTerm（当前任期）和votedId（本节点将选票投给了谁）
     *
     * @return
     */
    ...
    public boolean init(final NodeOptions opts) {
        ...
        this.voteTimer = new RepeatedTimer(name, this.options.getElectionTimeoutMs(), TIMER_FACTORY.getVoteTimer(
            ...
            protected void onTrigger() {
                //如果投票过程事件超时，那么会重新发起投票
                handleVoteTimeout();
            }

            @Override
            protected int adjustTimeout(final int timeoutMs) {
                //重置一个随机事件作为触发再次投票的计时事件
                return randomTimeout(timeoutMs);
            }
        };

        /**
         * 当leader在规定的一段时间内没有与 Follower 进行心跳通信时，
         * Follower 就可以认为leader异常，则 Follower 可以去尝试接替leader的角色。
         * 这段心跳超时被称为 Election Timeout，达到这个触发点后，Follower就会将角色转成候选人，并发起选举预投票
         *
         */
        ...
        /**
         * stepDownTimer定时检查是否需要重新选举leader。当前的leader可能出现它的Follower并没有占多大多数却还没有下台的情况，
         * 那么这个时候会定期的检查看leader的Follower数量是否占大多数，若没有，则会强制让leader下台。
         */
        ...
        /**
         * 初始化disruptor，采用多生产者模式，用于异步处理业务调用 Node#apply 方法向集群提交的 Task 列表
         */
        ...
        // closureQueue 在初始化 FSMCaller 时创建，相互共用，BallotBox 负责往 ClosureQueue 中写数据，而 FSMCaller 则负责从 ClosureQueue 中读数据。
        ...
        if (this.logManager.getLastLogIndex() > 0) {
            ...
            // initially set to max(priority of all nodes)，以初始节点中的最大优先级初始化 targetPriority，用于控制当前节点是否继续发起预选举
            this.targetPriority = getMaxPriorityOfNodes(this.conf.getConf().getPeers());
        }
        ...
        //创建 RPC 客户端，收其他节点或者客户端发过来的请求，转交给对应服务处理
        ...
        // set state to follower， 切换节点角色为 FOLLOWER
    ...
    private void stepDown(final long term, final boolean wakeupCandidate, final Status status) {
        ...
        if (this.state == State.STATE_CANDIDATE) {
            ...
            //如果是leader，现在已经stepDown了，那么停止stepDown检查定时器
            stopStepDownTimer();
            this.ballotBox.clearPendingTasks();
            // signal fsm leader stop immediately
            if (this.state == State.STATE_LEADER) {
                //如果当前节点是leader，那么处理stopLeader事件
                onLeaderStop(status);
            }
        }
        // reset leader_id，将leaderId置空
        resetLeaderId(PeerId.emptyPeer(), status);

        // soft state in memory，降级为Follower
        this.state = State.STATE_FOLLOWER;
        this.confCtx.reset();

        //由于更新leader，更新上一次leader的时间戳（结束时间）
        updateLastLeaderTimestamp(Utils.monotonicMs());
        //停止快照制作
        ...
        // meta state，如果是发现了其他任期更大的leader节点导致本leader节点stepDown，那么
        if (term > this.currTerm) {
            //采用更新的任期
            this.currTerm = term;
            //本节点没有投票给新leader，因此该字段置为空
    ...
    private void executeApplyingTasks(final List<LogEntryAndClosure> tasks) {
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                // 任务的期望任期与当前节点的任期进行比对，如果不满足条件，拒绝任务提交
    ...
    public void apply(final Task task) {
        ...
        switch (this.options.getApplyTaskMode()) {
            ...
            case NonBlocking:
            default:
                // 异步化发送日志，交给LogEntryAndClosureHandler处理
